...
LEVELS=${LEVELS:-9}            #maximum number of refinement levels allowed
...
#--------------------------------------
TDUMP=${TDUMP:-0.025}   #Save files for t = i*TDUMP, i=0,1,...
TEND=${TEND:-10.001}     #Perform simulation until t=TEND
...
OPTIONS="-bpdx $BPDX -bpdy $BPDY -levelMax $LEVELS -levelStart $LEVELSSTART -bAdaptChiGradient 1 -Rtol $RTOL -Ctol $CTOL -extent $EXTENT -CFL $CFL -tdump $TDUMP -nu $NU -tend $TEND -verbose $VERBOSE -maxPoissonIterations 1000 -maxPoissonRestarts 100 -poissonTol $PT -poissonTolRel $PTR -poissonSolver $PSOLVER"
OBJECTS="disk radius=$RADIUS xpos=$XPOS dumpSurf=1 tAccel=0 bForced=1 bFixed=1 xvel=$XVEL breakSymmetryStrength=$BREAK_SYMMETRY_STRENGTH breakSymmetryType=$BREAK_SYMMETRY_TYPE breakSymmetryTime=$BREAK_SYMMETRY_TIME"
